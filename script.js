 ...
 * Full-Stack Client (Vanilla JS + Supabase Authentication & Database)
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // =========================================================================
  // 1. Supabase Client & Application State
  // =========================================================================

  const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
  const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
  const supabaseConfigMissing = !supabaseUrl || !supabaseAnonKey || supabaseAnonKey === 'PASTE_YOUR_ANON_PUBLIC_KEY_HERE';

  if (supabaseConfigMissing) {
    console.error('[Supabase] Missing VITE_SUPABASE_URL or VITE_SUPABASE_ANON_KEY');
    showToast('Supabase is not configured. Add the Vercel environment variables.', 'error');
  }

  const supabase = supabaseConfigMissing ? null : createClient(supabaseUrl, supabaseAnonKey);
  ...
  function saveProfile() {
    // Synchronize athlete profile to Supabase if authenticated
    supabaseSyncProfile(athleteProfile);
  }
  ...
  /**
   * Upsert the in-memory athlete profile to the Supabase `athlete_profiles` table.
   * Called whenever the user saves settings or manually syncs.
   */
  async function supabaseSyncProfile(profile) {
    if (!supabase) return;
    let session;
    try {
      ({ data: { session } } = await supabase.auth.getSession());
    } catch (error) {
      console.warn('[Supabase Sync] Could not reach Supabase:', error.message);
      return;
    }
    if (!session) return;
    ...
    const { error } = await supabase
      ...
      .upsert({
        id: session.user.id,
        full_name: currentUser?.full_name || session.user.user_metadata?.full_name || null,
        ...
        first_day: profile.firstDay,
        weight: profile.weight,
        weight_unit: profile.weightUnit,
        height: profile.height,
        height_unit: profile.heightUnit,
        avatar_url: profile.avatarUrl,
        custom_image: profile.customImage,
        streak: profile.streak,
        completed_workouts: profile.completedWorkouts,
        total_target_workouts: profile.totalTargetWorkouts,
        consistency: profile.consistency,
        feedback_given: profile.feedbackGiven,
        updated_at: new Date().toISOString()
      }, { onConflict: 'id' });

    if (error) {
      console.warn('[Supabase Sync] Profile sync failed:', error.message);
    } else {
      console.log('[Supabase Sync] Profile saved to Supabase database.');
    }
  }

  /**
   * Validate the current Supabase session and load the user's profile from the
   * `athlete_profiles` table. Called on page load and after login/signup.
   */
  async function validateAndLoadUserSession() {
    ...
    try {
      ...
      console.warn('[Session Load] Could not reach Supabase:', error.message);
      showToast(getAuthErrorMessage(error, 'Could not check your Supabase session.'), 'error');
      updateAuthUI();
      return;
    }
    ...
    // Build the currentUser object to match what the rest of the UI expects
    currentUser = {
      id: session.user.id,
    ...
    try {
      // Load the athlete profile from Supabase
      ...
      if (profileData && !error) {
        // full_name on the profile row takes priority over metadata
        if (profileData.full_name) currentUser.full_name = profileData.full_name;
        athleteProfile = mapDbProfileToFrontend(profileData);
      }
    } catch (err) {
      console.warn('[Session Load] Could not load Supabase profile:', err.message);
    }
  ...
  /**
   * Log a completed workout. Stats are persisted via Supabase profile sync.
   * (Dedicated workout_sessions table support can be added later.)
   */
  async function apiLogWorkout(workoutData) {
    if (!supabase) return;
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    // Stats are already updated in athleteProfile by the caller; just sync.
    await supabaseSyncProfile(athleteProfile);
    console.log('[Supabase] Workout stats synced to Supabase profile.');
  }

  /**
   * Submit user feedback. Stored locally only – a dedicated feedback table
   * can be added to Supabase later without changing the UI.
   */
  async function apiSubmitFeedback(feedbackData) {
    console.log('[Feedback] Feedback noted (local only):', feedbackData);
  }
  ...
  function getAuthErrorMessage(error, fallback) {
    if (supabaseConfigMissing) return 'Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel.';

    const message = error?.message?.toLowerCase() || '';
    if (message.includes('invalid login credentials')) return 'Incorrect email or password.';
    if (message.includes('email not confirmed')) return 'Please confirm your email address before logging in.';
    ...
    if (message.includes('fetch') || message.includes('network') || message.includes('failed to') || error?.name === 'AuthRetryableFetchError') {
      return 'Unable to reach Supabase. Check your internet connection and Vercel environment variables.';
    }
    return error?.message || fallback;
  }

  function showAuthError(element, error, fallback) {
    if (!element) return;
    element.textContent = getAuthErrorMessage(error, fallback);
  ...
  if (authLoginForm) {
    authLoginForm.addEventListener('submit', async (e) => {
      ...
      if (!supabase) {
        showAuthError(loginErrorMsg, null, 'Login is unavailable until Supabase is configured.');
        return;
      }
      ...
      try {
        ...
        if (error) {
          showAuthError(loginErrorMsg, error, 'Login failed. Please verify your credentials.');
          return;
        }

        // Persist the current in-memory profile to Supabase, then load the saved row
        await supabaseSyncProfile(athleteProfile);
        await validateAndLoadUserSession();

        showToast(`Welcome back, ${currentUser?.full_name || 'Athlete'}!`, 'success');
        closeAuthModal();
      } catch (err) {
        showAuthError(loginErrorMsg, err, 'Login failed. Please try again.');
      } finally {
        if (submitBtn) submitBtn.disabled = false;
      }
    });
  ...
  if (authSignupForm) {
    authSignupForm.addEventListener('submit', async (e) => {
      ...
      if (!supabase) {
        showAuthError(signupErrorMsg, null, 'Sign up is unavailable until Supabase is configured.');
        return;
      }
      ...
      try {
        ...
        if (error) {
          showAuthError(signupErrorMsg, error, 'Sign up failed.');
          return;
        }

        // Check if Supabase requires email confirmation
        if (data.session === null) {
          // Email confirmation is enabled in Supabase – tell the user to check email
          closeAuthModal();
        ...
        // Sync the in-memory assessment profile to Supabase
        await supabaseSyncProfile(athleteProfile);
        await validateAndLoadUserSession();

        showToast('Account created and fitness profile saved to Supabase!', 'success');
        closeAuthModal();
      } catch (err) {
        showAuthError(signupErrorMsg, err, 'Sign up failed. Please try again.');
      } finally {
        if (submitBtn) submitBtn.disabled = false;
      }
    });
  ...
  if (authLogoutBtn) {
    authLogoutBtn.addEventListener('click', async () => {
      if (supabase) {
        ...
        if (error) {
          showToast(getAuthErrorMessage(error, 'Could not log out. Please try again.'), 'error');
          return;
        }
      }
      currentUser = null;
  ...
  if (authSyncBtn) {
    authSyncBtn.addEventListener('click', async () => {
      ...
      await supabaseSyncProfile(athleteProfile);
      await validateAndLoadUserSession();
      showToast('Profile synchronized with Supabase!', 'success');
    });
  }
  ...
  if (dashResetProfileBtn) {
    dashResetProfileBtn.addEventListener('click', async () => {
      if (confirm('Are you sure you want to reset your profile and stored workout metrics?')) {
        ...
        // Reset profile in Supabase if authenticated
        const { data: { session } } = supabase ? await supabase.auth.getSession() : { data: { session: null } };
        if (session) {
          try {
            await supabaseSyncProfile(athleteProfile);
          } catch (e) {
            console.warn('[Profile Reset] Could not reset on Supabase:', e.message);
          }
        }
  ...
  // Initial application startup: Validate Supabase session and load profile
